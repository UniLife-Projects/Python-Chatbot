...
def send():
    ...
    if val.url(response_predict):
        print('image')
        
        urllib.request.urlretrieve(response_predict,"image.jpeg")

        picture = plotImage.imread("image.jpeg")
        imgplot=plt.imshow(picture)
        plt.title("Image")
        plt.show()

        conclusion = "> Bot: " + empathy_response + " Our flickr development"
        txt.insert(END,"\n"+conclusion)
        
        
    else:
            
        conclusion = "> Bot: " + empathy_response + response_predict
        txt.insert(END, "\n" + conclusion)
    e.delete(0, END)
...
txt.insert(END, "Click 'Speak' to interact with the Customer Service ChatBot")
